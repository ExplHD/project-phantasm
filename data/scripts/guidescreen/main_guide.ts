...
export function Changelogs(player: Player) {
	const form = new ActionFormData()
		...
		.divider()
		.header("Changes")
		.label("= Added a cooldown between Legendary Weapon attacks (Solaris Verdant, Supercharged Copper Axe, Prism Weaver, Auric Photonizer, The Bleeding Spire, and Seiketsu) so attack patterns can no longer be spammed without a pause - ExplerHD")
		.label("= Fixed player scoreboard initialization: all scores are now reset when a player spawns (including weapon attack scores), so no leftover scores remain from previous sessions - ExplerHD")
		.divider()
		.header("Addition")
		.label("+ Upgraded several particle textures to use PBR (metalness/emissive/roughness) so they can glow and look smoother: circle_fade, circle_load, crosshair_warning, slash_effect_white, slash_effect_white_2, sonic_explosion_grayscaled, sparkle, and the Damage Indicator - ExplerHD")
		.divider()
		.header("v1.5.1")
		.divider()
		.header("Removal")
		.label("- Removed the debug log spam that printed on every ore mined - ExplerHD")
		.divider()
		.header("Changes")
		.label("= Rewrote the Dynamic Light System: no more flickering when switching items, lights are placed instantly, and they only spawn on air or liquid blocks so they won't break tall grass, flowers, or doors - ExplerHD")
		.label("= Health Bar now uses the native on-screen display, so it no longer spams the chat and shows correctly on respawn - ExplerHD")
		.label('= Fixed a crash ("setTitle of undefined") that occurred whenever a mob took damage - ExplerHD')
		.label("= Overhauled the Guidescreen: fixed wrong weapon titles and a wrong drop source, cleaned up typos, and corrected outdated HP data - ExplerHD")
		.label("= Fixed the Damage Indicator icons being mispositioned - ExplerHD")
		.label("= Moved the Dash cooldown scoreboard to be initialized when a player joins - ExplerHD")
		.divider()
		.header("Addition")
		.label("+ Added a Getting Started page to the Guidescreen with a recommended progression path, plus a welcome message on first join - ExplerHD")
		.label("+ Added a crafting recipe to turn a Rusted Fortune Coin into 4 Gold Blocks - ExplerHD")
		.divider()
		.header("v1.5.0")
		.divider()
		.header("Removal")
