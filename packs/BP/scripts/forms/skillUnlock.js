    ...
    const form = new MessageFormData()
        ...
        .show(player).then(r => {
            if (r.selection == 0) {
                ...
                if (!healthLevel || healthLevel < 3) {
                    player.setDynamicProperty("ph:health_level", healthLevel + 1);
                    player.runCommand(`effect @s health_boost infinite ${3 * player.getDynamicProperty("ph:health_level")}`)
...
export function propertiesCheck(player) {
